...
if(count($_POST)) {
  ...
  //echo "\n\n<!-- Mark 7: ".(microtime(true) - $time_start)."s -->\n\n";
   ...
   foreach($customers_data_all_intervals as $cd) {
    ...
    if($cd["active"]==1)  {
      ...
      if($cd['period']==60) $rebill_60_number++;
      
      if($cd['period']==30) $rebill_30_number++;
      
      
      } else {
      
        if($cd['period']==60) $cancelled_60_number++;
        
        if($cd['period']==30) $cancelled_30_number++;
      }
  ...
  foreach($decline_types as $dt) {$stats["declines_per_type"][$dt] = array("count"=>0,"decline_count"=>0,"sum"=>0,"percentage"=>0); }
 ...
 //echo "\n\n<!-- Mark 9: ".(microtime(true) - $time_start)."s -->\n\n";
  ...
  //get number of rebill periods between the start of the search period and the end of it
     ...
     if($periods > 10) {
      $stats["periods_".$rebill_period] = "10 (capped, actual number is ".$periods.")";
      $periods = 10;
     }
    ...
    //echo "<!-- \n M: ".$merchant["BankID"]."".print_r($upsells,1)." -->\n";
      ...
      if($bank_transaction["transaction_number"] > 0) {
		  ...
		  $stats["merchants"][] = array(
			...
			"upsell_amount" => round($bank_transaction["amount"] - $initials_amount - $bank_transaction_only_rebills["amount"],2),
...
$amount_jcb = 
$transaction_total =
$transaction_rebill_total =
$transaction_initial_total =
$transaction_upsell_total =
$transaction_visa = 
